import assert from 'node:assert/strict';
import { access, readFile } from 'node:fs/promises';
import test from 'node:test';

const routes = [
  'src/pages/index.astro',
  'src/pages/technology/index.astro',
  'src/pages/tools/index.astro',
  'src/pages/reading/index.astro',
  'src/pages/nature/index.astro',
  'src/pages/life/index.astro',
  'src/pages/archive/index.astro',
  'src/pages/about/index.astro',
  'src/pages/writing/index.astro',
  'src/pages/writing/[slug].astro',
  'src/pages/series/index.astro',
  'src/pages/series/[id].astro',
  'src/pages/bookshelf/index.astro',
  'src/pages/favorites/index.astro',
  'src/pages/projects/index.astro',
  'src/pages/now/index.astro',
  'src/pages/tags/index.astro',
  'src/pages/404.astro',
];

test('all public routes and compatibility redirects exist', async () => {
  await Promise.all(routes.map((route) => access(new URL(`../${route}`, import.meta.url))));
});

test('home and consolidated category pages are data driven', async () => {
  const home = await readFile(new URL('../src/pages/index.astro', import.meta.url), 'utf8');
  assert.match(home, /nowItems/);
  assert.match(home, /getCollection\(['"]series['"]/);
  assert.match(home, /最近更新/);
  assert.match(home, /正在做/);

  for (const route of ['technology', 'reading', 'life']) {
    const source = await readFile(new URL(`../src/pages/${route}/index.astro`, import.meta.url), 'utf8');
    assert.match(source, /getCollection\(['"]writing['"]/);
    assert.doesNotMatch(source, /WritingFilters/);
  }

  const writingList = await readFile(new URL('../src/components/WritingList.astro', import.meta.url), 'utf8');
  assert.match(writingList, /dated-list-item/);
});

test('public pages do not retain long-lived skeleton placeholders', async () => {
  for (const route of routes.filter((route) => !route.includes('[slug]') && !route.includes('[id]'))) {
    const source = await readFile(new URL(`../${route}`, import.meta.url), 'utf8');
    assert.doesNotMatch(source, /内容正在整理中|RSS 准备中/);
  }
});

test('core content pages filter drafts and sample content does not invent external links', async () => {
  for (const route of ['src/pages/technology/index.astro', 'src/pages/reading/index.astro', 'src/pages/life/index.astro']) {
    const source = await readFile(new URL(`../${route}`, import.meta.url), 'utf8');
    assert.match(source, /draft/);
  }
  const sample = await readFile(new URL('../src/content/writing/site-notes.md', import.meta.url), 'utf8');
  assert.doesNotMatch(sample, /https?:\/\//);
});
